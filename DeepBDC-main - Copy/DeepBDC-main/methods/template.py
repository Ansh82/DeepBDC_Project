...
class BaselineTrain(nn.Module):
    ...
    # Modify the feature_forward method in BaselineTrain class
    def feature_forward(self, x):
        out = self.feature.forward(x)
        if self.params.method in ['stl_deepbdc', 'meta_deepbdc']:
            # Check the dimension of output and reshape if necessary
            if len(out.shape) == 2:  # If output is already flattened to [batch_size, features]
                # Reshape to [batch_size, channels, 1, 1] before passing to dcov
                # Assuming out shape is [batch_size, feature_dim]
                out = out.view(out.size(0), out.size(1), 1, 1)
            out = self.dcov(out)
            out = self.dropout(out)
        elif self.params.method in ['protonet', 'good_embed']:
            if len(out.shape) == 4:  # If output is still in spatial form
